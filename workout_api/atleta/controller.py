...
@router.get("/{id}", 
             ...
             status_code=status.HTTP_200_OK,
             response_model=AtletaOut)
async def query_by_id(id: UUID4, 
                      db_session: DatabaseDependecy) -> AtletaOut:
...
@router.patch("/{id}", 
             ...
             status_code=status.HTTP_200_OK,
             response_model=AtletaOut)
async def update_atleta(id: UUID4, 
...
async def excluir_atleta(id: UUID4, 
                      db_session: DatabaseDependecy) -> None:
    atleta: AtletaModel|None = (
        await db_session.execute(
            select(AtletaModel).filter_by(id=id)
        )
